...
const ALLOWED_COLUMNS = {
    ...
    players: ['id', 'name', 'dni', 'number', 'position', 'photo_url', 'status', 'team_id', 'league_id', 'created_at'],
...
export function useSupabase(tableName, options = {}) {
    ...
    const sanitizePayload = useCallback((payload) => {
        ...
        if (clean.idNumber && !clean.dni) clean.dni = clean.idNumber;
        if (clean.id_number && !clean.dni) clean.dni = clean.id_number;
